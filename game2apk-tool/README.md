...
当前发布候选为工具 `v1.3.9`；生成 APK 默认 versionCode `8`、versionName `1.3.0`（由 7/1.2.0 升级），包含动态作弊菜单、可逆无敌、战斗胜负控制、事件回想传送、加密音频修复，以及签名产物命名、日文作弊标签简体中文文档翻译和集中失败恢复。
...
正文翻译组和作弊标签组各自执行 2% 失败率阈值：失败块占本组不超过 2%（含 2%）时继续构建，失败块保留原文并写入报告/TXT；超过 2% 才阻止产物生成。报告字段 `failureCount`、`failureRatio`、`continuedWithFailures` 会明确记录该决定。
