...
当前发布候选为 versionCode `6`、versionName `1.1.0`（由 5/1.0.4 升级），包含作弊器面板、事件回想传送和加密音频修复。
...
python .\game2apk-tool\scripts\game2apk.py run ".\仙肴圣餐超魔改 Ver22" `
  ...
  --version-code 6 --version-name 1.1.0
```
...
本次更新目标 APK 固定为 `com.game2apk.xianyaoshengcanver22`、versionCode `6`、versionName `1.1.0`。稳定 keystore 位于 `.state/signing/<applicationId>/`，密码不写入日志、报告、APK、dist 或子进程 argv。覆盖安装必须继续使用同一 applicationId、同一签名证书，并使用 `adb install -r`；不要卸载或清除应用数据，否则 WebView 存档无法保留。详见 `docs/storage-and-upgrade.md`。
