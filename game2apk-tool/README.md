...
当前发布候选为工具 `v1.3.7`；生成 APK 默认 versionCode `8`、versionName `1.3.0`（由 7/1.2.0 升级），包含动态作弊菜单、可逆无敌、战斗胜负控制、事件回想传送、加密音频修复，以及签名产物命名和严格简体中文作弊标签翻译批次恢复。
...
高级作弊菜单使用游戏 `data/System.json` 中的变量/开关标签，并且在检测到需要翻译时执行独立的 `zh-CN` 标签流程；已经是中文且没有日文信号的标签可直接保留，存在日文或确定英文单词时会送入严格校验。正文翻译仍然可选，因此中文游戏可以只翻译作弊菜单标签。标签存在非中文内容时，构建会要求确认向 DeepSeek 发送这些名称；没有确认或 API Key 会阻止构建，不会用分类编号冒充翻译。
...
翻译筛选规则：正文翻译只会把含非中文文本的消息块作为候选；纯中文正文不会发送到 API，也不会被改写。中英混合正文仍按完整对话提交以保留上下文，但原有汉字会被保护并原样恢复。作弊菜单是例外：只要检测到需要翻译的标签，实际显示的变量/开关标签会整体送入独立 `zh-CN` 校验流程，`SE/EXP/HP/MP/ATK/DEF/X/Y/ID` 等缩写可保留，确定的英文单词和日文内容不能原样通过。检查报告会分别显示正文候选数量与作弊标签状态。
