...
当前发布版本：`v1.5.6`。本版修复高级作弊标签预览后在最终构建阶段重复请求翻译：正文翻译不再改写动态作弊标签，构建会复用预览缓存并把结果写入构建副本；同时为超大 MV 项目加入 Android WebView 资源错误、控制台异常和渲染进程退出诊断。
...
当前发布版本：`v1.5.3`。本版补齐构建异常路径的中间副本清理，成功发布后不再在 `.work` 留下完整游戏副本；失败时仍保留可续做检查点。

当前发布版本为工具 `v1.5.3`；生成 APK 默认 versionCode `9`、versionName `1.4.0`（由 8/1.3.0 升级），包含动态作弊菜单、可逆无敌、战斗胜负控制、事件回想传送、加密音频修复、超大项目外部资源包、桌面布局预览和 APK 内运行时布局编辑，以及签名产物命名、日文作弊标签简体中文文档翻译、正文上下文 TXT 翻译、高级作弊变量选择、高级标签缓存隔离、高思考小批次保护和构建中间副本自动清理。
...
高级作弊菜单使用游戏 `data/System.json` 中的变量/开关标签，并且在检测到日文时执行独立的 `zh-CN` 文档标签流程；已经是中文或英文的标签可直接保留，较长日文词组会送入严格校验，单独助词仅在中文语境中允许保留。正文翻译仍然可选，因此中文游戏可以只翻译作弊菜单标签。标签存在日文内容时，构建会要求确认向 DeepSeek 发送这些名称；没有确认或 API Key 会阻止构建，不会用分类编号冒充翻译。源文档和结果文档保存在 `.state/`，不包含 API Key。
...
点击“检查项目”时，工具会只读统计可翻译文本中的汉字、假名和拉丁字符比例，并在页面显示语言判断。检测到已有中文时，DeepSeek 翻译默认关闭；即使检测结果不是中文，翻译也仍然是用户可选项，必须手动勾选并确认第三方传输后才会执行。检测不会把文本发送到网络。

如果浏览器报告 `Failed to fetch`，先确认 `game2apk-ui.exe` 和同目录后台仍在运行；构建/翻译任务进行时新版后台不会因心跳短暂丢失而自动退出，并会对任务状态轮询进行重试。连续重试仍失败时，请重新启动 `game2apk-ui.exe`。
