...
当前发布版本：`v1.3.12`。本版修复高级作弊变量预览读取大型正文缓存导致内存飙升的问题；作弊标签缓存独立保存，并按每批最多 96 个标签分段请求。

当前发布版本为工具 `v1.3.12`；生成 APK 默认 versionCode `8`、versionName `1.3.0`（由 7/1.2.0 升级），包含动态作弊菜单、可逆无敌、战斗胜负控制、事件回想传送、加密音频修复，以及签名产物命名、日文作弊标签简体中文文档翻译、正文上下文 TXT 翻译、高级作弊变量选择和高级标签预览缓存隔离。
