...
Android 候选需确认 applicationId `com.game2apk.xianyaoshengcanver22`、versionCode 2、versionName 1.0.1、同一证书、非 debuggable、无 INTERNET、默认 icon 非空、zipalign/apksigner 通过、assets 严格对账且无 save/secret/keystore。覆盖安装只允许 `adb install -r`；禁止卸载和清数据。完整证据见 `docs/06-security-remediation-and-rebuild.md`。
