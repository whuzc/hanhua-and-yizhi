...
v1.3.10 起，正文遇到长度截断会自动先关闭 thinking 重试、再二分批次恢复；构建前也会把实际显示的作弊菜单标签写入本地文本文档，集中翻译后按 ID 读取结果并回写。翻译失败块保留原文但不再按比例阻止产物生成；英文/代码标识原样保留，较长日文词组必须转成简体中文，单独助词可在中文语境中保留；旧便携版二进制不会获得此修复。
