...
高级作弊标签预览会复用本次已完成的项目检查，不再重新解析全部地图；预检只读取运行时可见的 `System.json` 变量/开关。大型项目因此不会因第二次全量扫描长时间占用内存或让浏览器误报连接断开。

v1.4.0 起，正文会按事件或数据库记录的上下文整合为本地 TXT/TSV 批次（默认最多 60 块或 12,000 个源字符），在长度截断时自动先关闭 thinking 重试、再沿上下文边界二分恢复；构建前也会把实际显示的作弊菜单标签写入本地文本文档，集中翻译后按 ID 读取结果并回写。高级作弊变量可在标签翻译完成后由用户全选、清空、搜索或逐项勾选。翻译失败块保留原文但不再按比例阻止产物生成；英文/代码标识原样保留，较长日文词组必须转成简体中文，单独助词可在中文语境中保留。高级标签预览使用独立缓存；low/关闭思考可使用 96 条文档批次，high/max 思考自动改为单路、每批最多 24 条，避免高思考请求同时占满内存；旧便携版二进制不会获得此修复。
...
- 当暂存 `www` 的估算体积接近 APK ZIP32 4 GiB 上限时，构建会自动切换为“APK + 外部资源包”：APK 只包含运行时，完整资源写入 ZIP64 `*-resources.g2ares`，页面在完成日志中显示桌面文件路径、SHA-256 和手机目标路径 `/Android/data/<applicationId>/files/game2apk/`。两个文件必须来自同一次构建；应用启动会检查资源包缺失、改名、放错目录或清单不匹配，并给出复制提示。
...
如果页面短暂显示 `Failed to fetch`，前端会保留任务编号并重试状态请求；后台在任务进行时不会因为短暂心跳丢失而触发空闲回收。兼容 `--web` 入口的默认空闲回收宽限为约 5 分钟，专用 `game2apk-ui.exe` 关闭时仍由父进程立即回收。连续重试仍失败通常表示 `game2apk-ui.exe` 或后台已退出，请重新启动启动器。
