...
v1.3.11 起，正文会按事件或数据库记录的上下文整合为本地 TXT/TSV 批次（默认最多 60 块或 12,000 个源字符），在长度截断时自动先关闭 thinking 重试、再沿上下文边界二分恢复；构建前也会把实际显示的作弊菜单标签写入本地文本文档，集中翻译后按 ID 读取结果并回写。高级作弊变量可在标签翻译完成后由用户全选、清空、搜索或逐项勾选。翻译失败块保留原文但不再按比例阻止产物生成；英文/代码标识原样保留，较长日文词组必须转成简体中文，单独助词可在中文语境中保留；旧便携版二进制不会获得此修复。
