...
class SecurityAndInputContractTests(unittest.TestCase):
    ...
    def test_update_identity_and_webview_storage_contract_are_stable(self) -> None:
        ...
        self.assertIn("CaseInsensitiveAssetsPathHandler", activity)
        asset_handler = (root / "templates" / "android-rpgmv" / "app" / "src" / "main" / "java" / "com" / "game2apk" / "rpgmv" / "CaseInsensitiveAssetsPathHandler.java").read_text(encoding="utf-8")
        self.assertIn("resolveCaseInsensitive", asset_handler)
        self.assertIn("equalsIgnoreCase", asset_handler)
