...
> 当前工具版本：`v1.3.13`（修复高级作弊 high/max 思考模式卡住和内存占满；高思考改为单路 24 条以内的小批次）
> [下载 Windows portable v1.3.13](https://github.com/whuzc/hanhua-and-yizhi/releases/tag/v1.3.13)
...
正文和作弊标签的失败块都会保留原文并继续生成产物，不再按失败比例阻断构建。报告会列出失败数量、比例和原因，方便直接在生成后的 APK 中审阅实际效果；配置、授权、取消和构建/签名错误仍会正常中止任务。
